...
export default function PracticeTiles({ practiceKey, onBack, onGoHome, onSelectTile, onNavigate }) {
    ...
    <div className="hp-wrap pp-page" style={practice.style}>
        ...
        <div className="hp-container">
          <div className="pp-hero-inner">
            ...
            <div className="pp-breadcrumb">
              ...
              <span className="pp-breadcrumb-current">{practice.heroTitle}</span>
            </div>
            <h1>{practice.heroIcon} {practice.heroTitle}</h1>
            <p>{practice.heroDesc}</p>
          </div>
        </div>
      </section>
      ...
      <div className="pp-quick">
        <div className="pp-quick-inner">
          <h2 style={{ fontSize: '1.15rem', fontWeight: 700, marginBottom: 6 }}>Select a Topic</h2>
          <p style={{ fontSize: '0.88rem', color: '#757575', marginBottom: 20 }}>Click on any topic below to explore its content.</p>
          <div className="pp-select-grid">
            {practice.tiles.map((t, i) => (
              <div key={i} className="pp-select-tile" onClick={() => onSelectTile(i)} style={{ background: t.bg, borderColor: t.border }}>
                <div className="pp-select-tile-icon" style={{ background: t.iconBg }}>{t.icon}</div>
                <h4>{t.label}</h4>
              </div>
            ))}
          </div>
